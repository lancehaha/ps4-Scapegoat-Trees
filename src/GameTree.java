...
public class GameTree {
    ...
    int findValue() {
        ...
        return findhelper(root, Player.ONE);
    }
    public int findhelper(TreeNode node, Player player){
        if (node.leaf) {
            return node.value;
        }
        int wanted = 0;
        if (player == Player.ONE) {
            ...
            for (TreeNode i : node.children) {
                int value = findhelper(i, Player.TWO);
                if (value > wanted) {
                    wanted = value;
                }
            }
            ...
            wanted = Integer.MIN_VALUE;
            for (TreeNode i : node.children) {
                int value = findhelper(i, Player.ONE);
                if (value < wanted) {
                    wanted = value;
                }
            }
